...
public class RouteReactorImplTest {
    ...
    public void updateRouting() throws Exception {
        ...
        Mockito.when(rTx.read(Matchers.any(), Matchers.any()))
                .thenAnswer(invocation -> FluentFutures.immediateFluentFuture(Optional.of(oldRoute)));
        Mockito.when(wTx.commit()).thenAnswer(invocation -> FluentFutures.immediateNullFluentFuture());
