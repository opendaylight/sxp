...
public class SxpDatabaseImpl extends org.opendaylight.sxp.util.database.SxpDatabase {
    ...
    protected List<SxpDatabaseBinding> getBindings(SxpDatabaseBinding.BindingType bindingType) {
        List<SxpDatabaseBinding> output = new ArrayList<>();
        for (List<SxpDatabaseBinding> bindingList : bindingMap.values()) {
            for (SxpDatabaseBinding binding : bindingList) {
                if (binding.getBindingType() == bindingType) {
                    output.add(binding);
                }
            }
        }
