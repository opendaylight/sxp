...
public final class Search {
    ...
    public static <T extends SxpBindingFields> List<T> expandBinding(T binding, int quantity) {
        ...
        InetAddress max = InetAddresses.increment(IetfInetUtil.INSTANCE
            .inetAddressFor(IetfInetUtil.INSTANCE.ipAddressFor(bitSet.toByteArray())));
        for (InetAddress inetAddress =
             IetfInetUtil.INSTANCE.inetAddressFor(IetfInetUtil.INSTANCE.ipAddressFor(address));
             quantity > 0 && !max.equals(inetAddress);
             inetAddress = InetAddresses.increment(inetAddress), quantity--) {
