...
public class SxpDomain implements AutoCloseable {
    ...
    /**
     * @return Set of Domains to which this domain propagate bindings
     */
    private Set<String> getSharedDomains() {
        Set<String> domains = new HashSet<>();
        synchronized (filters) {
            filters.values()
                    .forEach(m -> m.values()
                            .forEach(f -> f.getSxpFilter()
                                    .getDomains()
                                    .getDomain()
                                    .forEach(d -> domains.add(d.getName()))));
        }
        return domains;
    }
    ...
    private List<SxpBindingFields> shared(final List<SxpBindingFields> shared,
            final Predicate<SxpBindingFields> filter) {
     ...
     * @param domains Domains from which bindings for replace will be
     */
    private void propagateToSharedDomain(final List<SxpBindingFields> removed, final List<SxpBindingFields> added,
        ...
        if (domain == null || this.name.equals(domain.getName()) || (removed.isEmpty() && added.isEmpty()))
            return;
        List<SxpBindingFields> add = new ArrayList<>(added);
        List<MasterDatabaseBinding> deleted = new ArrayList<>();
        synchronized (domain.getSxpDatabase()) {
            //Find Replace for bindings
            if (!removed.isEmpty()) {
                deleted = domain.getMasterDatabase().deleteBindings(removed);

                List<SxpBindingFields> replace = new ArrayList<>(domain.getSxpDatabase().getBindings());
                replace.addAll(domain.getMasterDatabase().getLocalBindings());

                domains.stream().forEach(d -> d.shared(replace, d.getFilters().get(domain.getName())));
                add.addAll(SxpDatabase.getReplaceForBindings(removed, replace));
            }
            //Propagate changes
            node.getSvcBindingDispatcher()
                    .propagateUpdate(deleted, domain.getMasterDatabase().addBindings(add), domain.getConnections()
                            .stream()
    ...
    public void propagateToSharedDomain(SxpDomain domain) {
        if (domain != null && !this.equals(domain))
            propagateToSharedDomain(Collections.emptyList(),
                    shared(new ArrayList<>(), getFilters().get(Preconditions.checkNotNull(domain.getName()))),
            ...
            if (sxpDomains.containsKey(filter.getKey()))
                propagateToSharedDomain(removed.stream().filter(filter.getValue()).collect(Collectors.toList()),
                        ...
                        sxpDomains.get(filter.getKey()), sxpDomains.values()
                                .stream()
                                .filter(d -> d.getSharedDomains().contains(filter.getKey()))
                                .collect(Collectors.toList()));
        }
    }
