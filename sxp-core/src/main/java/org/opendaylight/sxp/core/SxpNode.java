...
public class SxpNode {
    ...
    /**
     * Updates TCP-MD5 keys of SxpNode.
     * This is method is thread safe and uses fast path optimization to avoid unnecessary server restarts in high-load
     * situations.
     * As a result, two situations may occur when calling this method. If the calling thread:
     * 1. is the first to arrive at the critical section, it sets processing flag on and restart the server with new keyset
     * 2. invokes the method while another thread is
     * 2.a: before passwords extracted step then it leaves
     * 2.b: after passwords are extracted then the processing flag is off and this thread sets this flag back on
     *      and waits until the previous restart is done - then continues with new restart
     *
    ...
    private void doUpdateMD5keys(@Nullable SxpConnection con) {
        ...
        bindServerFuture.updateAndGet(listenableFuture -> {
            if (isEnabled() && listenableFuture != null) {
                if (listenableFuture.isDone()) {
                    // exclusive password extraction from all node connections
                    md5UpdateLock.lock();
                    //revoke processing flag
                    processingConnectionData = false;
                    //read keyMap
                    final Map<InetAddress, byte[]> keyMapping = ConnectFacade.collectAllPasswords(this);
                    md5UpdateLock.unlock();
                    ...
                    synchronized (md5UpdateLock) {
                        ...
                        serverChannel
                                = ConnectFacade.createServer(this, handlerFactoryServer, keyMapping).awaitUninterruptibly().channel();
                    }
                }
            }
            return listenableFuture;
