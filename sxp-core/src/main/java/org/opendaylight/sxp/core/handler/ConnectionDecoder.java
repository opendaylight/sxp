...
@Sharable public class ConnectionDecoder extends SimpleChannelInboundHandler<ByteBuf> {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(ConnectionDecoder.class.getName());
     ...
     */
    protected void addConnection(SxpNode node, String domain, Connection connection) {
        node.addConnection(connection, domain);
    }
    ...
    @Override public void channelRead0(ChannelHandlerContext ctx, ByteBuf message)
        ...
        if (address != null && connection == null && domain != null) {
            ...
            if ((template.getTemplateMode() == null || template.getTemplateMode().equals(mode)) && (
                ...
                addConnection(owner, domain.getName(), new ConnectionBuilder().setMode(invertMode(mode))
