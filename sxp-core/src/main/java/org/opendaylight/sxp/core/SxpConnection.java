...
public class SxpConnection {
    ...
    private final Deque<Callable<?>> inboundUpdateMessageQueue = new ArrayDeque<>(10),outboundUpdateMessageQueue= new ArrayDeque<>(10);
    private final AtomicReference<ListenableFuture<?>> inboundUpdateMessageExecutor = new AtomicReference<>(), outboundUpdateMessageExecutor = new AtomicReference<>();
    ...
    /**
     * Test if has some Update Messages to export.
     *
     * @return true if have massages to export
     */
    public boolean hasUpdateMessageOutbound() {
        synchronized (outboundUpdateMessageQueue) {
            return !outboundUpdateMessageQueue.isEmpty();
        }
    }
    ...
    /**
     * Test if has some Update Messages to import.
     *
     * @return true if have massages to import
     */
    public boolean hasUpdateMessageInbound() {
        synchronized (inboundUpdateMessageQueue) {
            return !inboundUpdateMessageQueue.isEmpty();
        }
    }
    ...
    /**
     * Gets reference to task executing import of Update Messages.
     *
     * @return Reference to task that is/was importing Update Message
     */
    public AtomicReference<ListenableFuture<?>> getInboundUpdateMessageExecutor() {
        return inboundUpdateMessageExecutor;
    }

    /**
     * Gets reference to task executing export of Update Messages.
     *
     * @return Reference to task that is/was exporting Update Message
     */
    public AtomicReference<ListenableFuture<?>> getOutboundUpdateMessageExecutor() {
        return outboundUpdateMessageExecutor;
    }
    ...
    public void shutdown() {
        if (isModeListener()){
            try {
                clearMessages();
                ListenableFuture<?> updateMessageExecutor_ = inboundUpdateMessageExecutor.getAndSet(null);
                if(updateMessageExecutor_!=null && !updateMessageExecutor_.isDone()){
                    updateMessageExecutor_.cancel(false);
                }
                updateMessageExecutor_ = outboundUpdateMessageExecutor.getAndSet(null);
                if(updateMessageExecutor_!=null && !updateMessageExecutor_.isDone()){
                    updateMessageExecutor_.cancel(false);
                }
