...
public class SxpConnection {
    ...
    public void stopRetryOpenTimer() {
        LOG.warn("{} acquiring lock 0", this);
        retryOpenSchedulerLock.lock();
        try {
            LOG.warn("{} acquired lock 0", this);
            if (scheduledRetryOpen != null) {
                scheduledRetryOpen.cancel(true);
            }
        } finally {
            LOG.warn("{} unlocking lock 0", this);
    ...
    public void scheduleRetryOpen() {
        ...
        LOG.warn("{} acquiring lock 1", this);
        retryOpenSchedulerLock.lock();
        try {
            LOG.warn("{} acquired lock 1", this);
            ...
            scheduledRetryOpen.addListener(() -> { // when a timer has expired
                LOG.warn("{} acquiring lock 2", this);
                retryOpenSchedulerLock.lock();
                try {
                LOG.warn("{} acquired lock 2", this);
                    ...
                    connectFuture.addListener((FutureListener<Void>) future -> {
                        LOG.warn("{} acquiring lock 3", this);
                        retryOpenSchedulerLock.lock();
                        try {
                            LOG.warn("{} acquired lock 3", this);
                            ...
                            LOG.warn("{} unlocking lock 4", this);
                            retryOpenSchedulerLock.unlock();
                        }
                    });
                    ...
                    LOG.warn("{} unlocking lock 5", this);
                    retryOpenSchedulerLock.unlock();
                }
            }, MoreExecutors.directExecutor());
        } finally {
            LOG.warn("{} unlocking lock 6" , this);
            retryOpenSchedulerLock.unlock();
        }
    }
