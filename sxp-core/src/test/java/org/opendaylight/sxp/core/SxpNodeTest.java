...
public class SxpNodeTest {
    ...
    public void testOpenConnections() throws Exception {
        ...
        verify(worker, times(4)).executeTask(any(Runnable.class), any(ThreadsWorker.WorkerType.class));
        node.addConnection(mockConnection(ConnectionMode.Speaker, ConnectionState.Off));
        node.openConnections();
        verify(worker, times(5)).executeTask(argument.capture(), any(ThreadsWorker.WorkerType.class));
