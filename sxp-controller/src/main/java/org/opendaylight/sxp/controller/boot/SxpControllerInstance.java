...
public class SxpControllerInstance implements ClusterSingletonService, AutoCloseable {
     ...
     */
    private synchronized void initBindingOrigins(DatastoreAccess datastoreAccess, Map<OriginType, Integer> defaultOriginPriorities,
            LogicalDatastoreType datastoreType) {
        final InstanceIdentifier<BindingOrigins> containerPath = InstanceIdentifier.builder(BindingOrigins.class)
                .build();
        final BindingOrigins origins = new BindingOriginsBuilder().build();

        // ensure empty container binding origins exists, but do not override it
        datastoreAccess.checkAndPut(containerPath, origins, datastoreType, false);
        ...
        defaultOriginPriorities.forEach((originType, priority) -> {
            ...
            datastoreAccess.checkAndPut(listPath, origin.build(), datastoreType, false);
        });
    }
