...
public abstract class SxpDataChangeListener<T extends DataObject> implements DataTreeChangeListener<T> {
    ...
    protected <R> R checkChange(DataTreeModification<T> c, Function<T, R> function) {
        ...
        return function.apply(before).equals(function.apply(after)) ? null : function.apply(after);
    }

    protected abstract void handleConfig(DataTreeModification<T> c, final String nodeId);
