...
public class NodeIdentityListener implements DataTreeChangeListener<SxpNodeIdentity> {
    ...
    private final List<SxpDataChangeListener<?>> subListeners;
    ...
    public NodeIdentityListener(DatastoreAccess datastoreAccess) {
        ...
        subListeners.add(new ConnectionsListener(this.datastoreAccess));
        subListeners.add(new PeerGroupListener(this.datastoreAccess));
        subListeners.add(new MasterBindingListener(this.datastoreAccess));
    }
                    ...
                    case SUBTREE_MODIFIED:
                        if (c.getRootNode().getDataAfter() != null) {
                            subListeners.forEach(l -> {
                                l.handleChange(c.getRootNode().getModifiedChildContainer(l.getWatchedContainer()),
                                        c.getRootPath().getDatastoreType(), c.getRootPath().getRootIdentifier());
                            });
                            break;
                        }
                    case DELETE:
