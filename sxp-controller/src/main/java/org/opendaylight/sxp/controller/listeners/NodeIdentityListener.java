...
import static org.opendaylight.sxp.controller.core.RpcServiceImpl.getDatastoreAccess;
        ...
        changes.stream().forEach(c -> {
            ...
            if (LogicalDatastoreType.CONFIGURATION.equals(c.getRootPath().getDatastoreType())) {
                switch (c.getRootNode().getModificationType()) {
                    ...
                    case DELETE:
                        getDatastoreAccess(nodeId, datastoreAccess).checkAndDelete(c.getRootPath().getRootIdentifier(),
                ...
                switch (c.getRootNode().getModificationType()) {
                    case WRITE:
                        if (c.getRootNode().getDataBefore() == null)
                            ...
                            RpcServiceImpl.unregisterNode(Preconditions.checkNotNull(nodeId)).shutdown();
                            break;
                        }
                    case SUBTREE_MODIFIED:
                        if (checkDifference(c, SxpNodeFields::isEnabled)) {
                            if (Preconditions.checkNotNull(c.getRootNode().getDataAfter()).isEnabled()) {
                                RpcServiceImpl.getNode(nodeId).start();
                            } else {
                                RpcServiceImpl.getNode(nodeId).shutdown();
                            }
                            ...
                            RpcServiceImpl.getNode(nodeId).shutdown().start();
                        } else if (checkDifference(c, SxpNodeIdentityFields::getTimers)) {
                            RpcServiceImpl.getNode(nodeId).shutdownConnections();
                        }
                        subListeners.forEach(l -> {
                            l.handleChange(l.getModifications(c), c.getRootPath().getDatastoreType(),
                                    c.getRootPath().getRootIdentifier());
                        ...
                        break;
                    case DELETE:
                        RpcServiceImpl.unregisterNode(Preconditions.checkNotNull(nodeId)).close();
                        break;
                }
            }
        });
