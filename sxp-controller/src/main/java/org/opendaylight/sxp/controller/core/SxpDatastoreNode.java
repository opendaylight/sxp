...
public class SxpDatastoreNode extends org.opendaylight.sxp.core.SxpNode {
    ...
    public static InstanceIdentifier.InstanceIdentifierBuilder<SxpNodeIdentity> getIdentifierBuilder(
            final String nodeId) {
    ...
    public SxpDatastoreNode(NodeId nodeId, DatastoreAccess datastoreAccess, SxpNodeIdentity node) {
        super(Preconditions.checkNotNull(nodeId), node,
    ...
    @Override protected synchronized SxpNodeIdentity getNodeIdentity() {
        return datastoreAccess.readSynchronous(getIdentifierBuilder(nodeId).build(), LogicalDatastoreType.OPERATIONAL);
    }
    ...
    @Override protected synchronized Security setPassword(Security security) {
        SxpNodeIdentityBuilder
                identityBuilder =
                new SxpNodeIdentityBuilder(datastoreAccess.readSynchronous(getIdentifierBuilder(nodeId).build(),
                        LogicalDatastoreType.CONFIGURATION));

        datastoreAccess.putSynchronous(getIdentifierBuilder(nodeId).build(),
                identityBuilder.setSecurity(super.setPassword(security)).build(), LogicalDatastoreType.OPERATIONAL);
        return identityBuilder.getSecurity();
    }
}
