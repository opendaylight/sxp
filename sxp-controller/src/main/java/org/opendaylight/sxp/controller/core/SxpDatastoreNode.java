...
public class SxpDatastoreNode extends org.opendaylight.sxp.core.SxpNode {
    ...
    @Override protected synchronized SxpNodeIdentity getNodeIdentity() {
        SxpNodeIdentity
                identity =
                datastoreAccess.readSynchronous(getIdentifierBuilder(nodeId).build(), LogicalDatastoreType.OPERATIONAL);
    ...
    @Override protected synchronized Security setPassword(Security security) {
        ...
        SxpNodeIdentity
                ...
                datastoreAccess.readSynchronous(getIdentifierBuilder(nodeId).build(), LogicalDatastoreType.OPERATIONAL);
        if (identity != null) {
            SxpNodeIdentityBuilder identityBuilder = new SxpNodeIdentityBuilder(identity);
            identityBuilder.setSecurity(nodeSecurity);
            datastoreAccess.put(getIdentifierBuilder(nodeId).build(), identityBuilder.build(),
