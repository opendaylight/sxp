...
public class SxpDatastoreNode extends org.opendaylight.sxp.core.SxpNode implements AutoCloseable {
    ...
    @Override public void setSecurity(final Security security) {
        super.setSecurity(security);
        if (datastoreAccess != null) {
            datastoreAccess.checkAndPut(getIdentifier(nodeId).child(Security.class),
                    super.getNodeIdentity().getSecurity(), LogicalDatastoreType.OPERATIONAL, true);
        }
    }
