...
public final class ConfigLoader {
    ...
    public void load(Topology topology) {
        if (topology == null || topology.getNode() == null || topology.getNode().isEmpty()) {
            return;
        }
        topology.getNode().stream().forEach(n -> {
            initTopologyNode(Preconditions.checkNotNull(n.getNodeId()).getValue(), LogicalDatastoreType.OPERATIONAL,
                    datastoreAccess);
        });
    }
