...
public class PeerGroupListener extends SxpDataChangeListener<SxpPeerGroup> {
    ...
    public PeerGroupListener(DatastoreAccess datastoreAccess) {
        ...
        filterListener = new FilterListener(this.datastoreAccess);
    }
    ...
    @Override protected void handleOperational(DataObjectModification<SxpPeerGroup> c,
        ...
        LOG.trace("Operational Modification PeerGroups {}", c.getModificationType());
        switch (c.getModificationType()) {
            ...
            case SUBTREE_MODIFIED:
                ...
                c.getModifiedChildren().forEach(m -> {
                    if (SxpFilter.class.equals(m.getDataType())) {
                        //noinspection unchecked
                        filterListener.handleOperational((DataObjectModification<SxpFilter>) m,
                                getIdentifier(c.getDataAfter(), identifier));
                    }
                });
                break;
            case DELETE:
    ...
    @Override protected void handleConfig(DataObjectModification<SxpPeerGroup> c,
            InstanceIdentifier<SxpNodeIdentity> identifier) {
        switch (c.getModificationType()) {
            case WRITE:
            ...
            case SUBTREE_MODIFIED:
                if (c.getDataAfter() != null) {
                    c.getModifiedChildren().forEach(m -> {
                        if (SxpFilter.class.equals(m.getDataType())) {
                            //noinspection unchecked
                            filterListener.handleConfig((DataObjectModification<SxpFilter>) m,
                                    getIdentifier(c.getDataAfter(), identifier));
                        }
                    });
                    break;
    ...
    @Override public <R extends DataObject> Class<R> getWatchedContainer() {
        //noinspection unchecked
        return (Class<R>) SxpPeerGroups.class;
    }
