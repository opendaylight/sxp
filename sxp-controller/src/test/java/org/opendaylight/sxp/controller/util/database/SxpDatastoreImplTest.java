...
@RunWith(PowerMockRunner.class) @PrepareForTest({DatastoreAccess.class})
public class SxpDatastoreImplTest {
        ...
        @BeforeClass public static void initClass() {
                ...
                PowerMockito.when(access
                    .mergeSynchronous(any(InstanceIdentifier.class), any(BindingSource.class),
                        any(LogicalDatastoreType.class))).then(invocation -> {
                        InstanceIdentifier identifier =
                            (InstanceIdentifier) invocation.getArguments()[0];
                        if (identifier.getTargetType() == BindingSource.class) {
                                Map<NodeId, List<SxpDatabaseBinding>> map =
                                    ((BindingDatabaseKey) identifier
                                        .firstKeyOf(BindingDatabase.class)).getBindingType()
                                        == BindingDatabase.BindingType.ActiveBindings ?
                                        databaseBindings_Active :
                                        databaseBindings_Reconciled;
                                NodeId nodeId =
                                    ((BindingSourceKey) identifier.firstKeyOf(BindingSource.class))
                                        .getSourceId();

                                if (!map.containsKey(nodeId)) {
                                        map.put(nodeId, new ArrayList<>());
                                }
                                map.get(nodeId).addAll(
                                    ((BindingSource) invocation.getArguments()[1])
                                        .getSxpDatabaseBindings().getSxpDatabaseBinding());
                        }
                        return null;
                });
                PowerMockito.when(access
                    .putSynchronous(any(InstanceIdentifier.class), any(MasterDatabase.class),
                        any(LogicalDatastoreType.class))).then(invocation -> {
                        InstanceIdentifier identifier =
                            (InstanceIdentifier) invocation.getArguments()[0];
                        if (identifier.getTargetType() == BindingSource.class) {
                ...
                PowerMockito.when(access.deleteSynchronous(any(InstanceIdentifier.class),
                    any(LogicalDatastoreType.class))).then(invocation -> {
                        InstanceIdentifier identifier =
                            (InstanceIdentifier) invocation.getArguments()[0];
                        if (identifier.getTargetType() == BindingSource.class) {
                ...
                PowerMockito.when(access.readSynchronous(any(InstanceIdentifier.class),
                    any(LogicalDatastoreType.class))).then(invocation -> {
                        InstanceIdentifier identifier =
                            (InstanceIdentifier) invocation.getArguments()[0];
                        Map<NodeId, List<SxpDatabaseBinding>> map =
                        ...
                        if (identifier.getTargetType() == BindingSource.class) {
                                ...
                                return new BindingSourceBuilder().setSourceId(nodeId)
                                    .setSxpDatabaseBindings(new SxpDatabaseBindingsBuilder()
                                        .setSxpDatabaseBinding(map.containsKey(nodeId) ?
                                            map.get(nodeId) :
                                            new ArrayList<SxpDatabaseBinding>()).build()).build();
                        } else if (identifier.getTargetType() == BindingDatabase.class) {
                                List<BindingSource> bindingSources = new ArrayList<BindingSource>();
                                map.entrySet().stream().forEach(e -> {
                                        bindingSources.add(
                                            new BindingSourceBuilder().setSourceId(e.getKey())
                                                .setSxpDatabaseBindings(
                                                    new SxpDatabaseBindingsBuilder()
                                                        .setSxpDatabaseBinding(e.getValue())
                                                        .build()).build());
                                });
                                return new BindingDatabaseBuilder().setBindingSources(
                                    new BindingSourcesBuilder().setBindingSource(bindingSources)
                                        .build()).build();
                        }
                        return null;
                });
