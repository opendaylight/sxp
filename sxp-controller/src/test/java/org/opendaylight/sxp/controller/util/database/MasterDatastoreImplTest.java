...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.controller.md.sal.core.spi.operational.dom.store.rev140617.OperationalDomDatastore;
...
public class MasterDatastoreImplTest {
        ...
        @BeforeClass public static void initClass() {
                ...
                PowerMockito.when(access
                    .mergeSynchronous(any(InstanceIdentifier.class), any(MasterDatabase.class),
                        any(LogicalDatastoreType.class))).then(invocation -> {
                        Map<IpPrefix, MasterDatabaseBinding> databaseBindings =
                            ((LogicalDatastoreType) invocation.getArguments()[2])
                                == LogicalDatastoreType.OPERATIONAL ?
                                databaseBindings_Op :
                                databaseBindings_Conf;

                        ((MasterDatabase) invocation.getArguments()[1]).getMasterDatabaseBinding()
                            .stream().forEach(b -> {
                ...
                PowerMockito.when(access.readSynchronous(any(InstanceIdentifier.class),
                    any(LogicalDatastoreType.class))).then(invocation -> {
                        Map<IpPrefix, MasterDatabaseBinding> databaseBindings =
                            ((LogicalDatastoreType) invocation.getArguments()[1])
                                == LogicalDatastoreType.OPERATIONAL ?
                                databaseBindings_Op :
                                databaseBindings_Conf;

                        if (((InstanceIdentifier) invocation.getArguments()[0]).getTargetType()
                            == MasterDatabase.class) {
                                return new MasterDatabaseBuilder().setMasterDatabaseBinding(
                                    new ArrayList<>(databaseBindings.values())).build();
                        } else if (
                            ((InstanceIdentifier) invocation.getArguments()[0]).getTargetType()
                                == MasterDatabaseBinding.class) {
                                return databaseBindings.get(
                                    ((MasterDatabaseBindingKey) ((InstanceIdentifier) invocation
                                        .getArguments()[0]).firstKeyOf(MasterDatabaseBinding.class))
                                        .getIpPrefix());
                        }
                        return null;
                });
